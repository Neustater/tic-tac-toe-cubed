...
function Square({ value, onSquareClick, squareInd, boardInd}) {
  return (
    <button className="square" style={{fontSize: (45/boardDimentions) + "dvh", lineHeight: (45/boardDimentions) + "dvh"}} onClick={() => onSquareClick(boardInd, squareInd)}>
      {value}
    </button>
  );
}
...
function Board({boardInd, board, onSquareClick, sideResult}) {
  ...
  function renderBoard() {
    ...
    for (let squareInd = 0; squareInd < boardDimentions * boardDimentions; squareInd++) {
      boardElement.push(
        <Square key={squareInd} value={board[squareInd]} boardInd={boardInd} squareInd={squareInd} onSquareClick={onSquareClick} />
      );
    }
    return boardElement;
  }
